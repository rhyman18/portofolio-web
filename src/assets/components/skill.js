...
if (skillDasar.data.length > 0) {
  document.getElementById('skeleton-box-basic').style.display = 'none';
}

for (const skill of skillDasar.data) {
  const createSkill = document.createElement('div');
...
if (skillFrontend.data.length > 0) {
  document.getElementById('skeleton-box-frontend').style.display = 'none';
}
...
if (skillBackend.data.length > 0) {
  document.getElementById('skeleton-box-backend').style.display = 'none';
}
